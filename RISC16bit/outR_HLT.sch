<?xml version="1.0" encoding="UTF-8"?>
<drawing version="7">
    <attr value="spartan6" name="DeviceFamilyName">
        <trait delete="all:0" />
        <trait editname="all:0" />
        <trait edittrait="all:0" />
    </attr>
    <netlist>
        <signal name="instruction0" />
        <signal name="outR_HLT" />
        <signal name="HLT" />
        <signal name="outR" />
        <port polarity="Input" name="instruction0" />
        <port polarity="Input" name="outR_HLT" />
        <port polarity="Output" name="HLT" />
        <port polarity="Output" name="outR" />
        <blockdef name="and2">
            <timestamp>2000-1-1T10:10:10</timestamp>
            <line x2="64" y1="-64" y2="-64" x1="0" />
            <line x2="64" y1="-128" y2="-128" x1="0" />
            <line x2="192" y1="-96" y2="-96" x1="256" />
            <arc ex="144" ey="-144" sx="144" sy="-48" r="48" cx="144" cy="-96" />
            <line x2="64" y1="-48" y2="-48" x1="144" />
            <line x2="144" y1="-144" y2="-144" x1="64" />
            <line x2="64" y1="-48" y2="-144" x1="64" />
        </blockdef>
        <blockdef name="and2b1">
            <timestamp>2000-1-1T10:10:10</timestamp>
            <line x2="64" y1="-48" y2="-144" x1="64" />
            <line x2="144" y1="-144" y2="-144" x1="64" />
            <line x2="64" y1="-48" y2="-48" x1="144" />
            <arc ex="144" ey="-144" sx="144" sy="-48" r="48" cx="144" cy="-96" />
            <line x2="192" y1="-96" y2="-96" x1="256" />
            <line x2="64" y1="-128" y2="-128" x1="0" />
            <line x2="40" y1="-64" y2="-64" x1="0" />
            <circle r="12" cx="52" cy="-64" />
        </blockdef>
        <block symbolname="and2" name="XLXI_5">
            <blockpin signalname="outR_HLT" name="I0" />
            <blockpin signalname="instruction0" name="I1" />
            <blockpin signalname="HLT" name="O" />
        </block>
        <block symbolname="and2b1" name="XLXI_7">
            <blockpin signalname="instruction0" name="I0" />
            <blockpin signalname="outR_HLT" name="I1" />
            <blockpin signalname="outR" name="O" />
        </block>
    </netlist>
    <sheet sheetnum="1" width="3520" height="2720">
        <branch name="instruction0">
            <wire x2="400" y1="192" y2="192" x1="240" />
            <wire x2="432" y1="192" y2="192" x1="400" />
            <wire x2="400" y1="192" y2="416" x1="400" />
            <wire x2="432" y1="416" y2="416" x1="400" />
        </branch>
        <iomarker fontsize="28" x="240" y="192" name="instruction0" orien="R180" />
        <iomarker fontsize="28" x="240" y="256" name="outR_HLT" orien="R180" />
        <instance x="432" y="320" name="XLXI_5" orien="R0" />
        <branch name="outR_HLT">
            <wire x2="384" y1="256" y2="256" x1="240" />
            <wire x2="432" y1="256" y2="256" x1="384" />
            <wire x2="384" y1="256" y2="480" x1="384" />
            <wire x2="432" y1="480" y2="480" x1="384" />
        </branch>
        <instance x="432" y="352" name="XLXI_7" orien="M180" />
        <branch name="HLT">
            <wire x2="720" y1="224" y2="224" x1="688" />
        </branch>
        <iomarker fontsize="28" x="720" y="224" name="HLT" orien="R0" />
        <branch name="outR">
            <wire x2="720" y1="448" y2="448" x1="688" />
        </branch>
        <iomarker fontsize="28" x="720" y="448" name="outR" orien="R0" />
    </sheet>
</drawing>